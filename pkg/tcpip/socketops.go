...
type SocketOptionsHandler interface {
	...
	// WakeupWritersOnSetSendBufferSize is invoked when the send buffer size for an
	...
	// is increased.
	WakeupWritersOnSetSendBufferSize()
}
...
// WakeupWritersOnSetSendBufferSize implements SocketOptionsHandler.WakeupWritersOnSetSendBufferSize.
func (*DefaultSocketOptionsHandler) WakeupWritersOnSetSendBufferSize() {}
...
func (so *SocketOptions) SetSendBufferSize(sendBufferSize int64, notify bool) {
	...
	if oldSndBufSz < sendBufferSize {
		so.handler.WakeupWritersOnSetSendBufferSize()
	}
}
